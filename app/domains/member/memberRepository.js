...
class MemberRepository {
    async getAllMembers() {
        try {
            return await Member.find({}, {__v:0, _id:0}).sort({code:1});
        } catch (error) {
    ...
    async getMemberByCode(code) {
        try {
            return await Member.findOne({code}, {__v:0, _id:0});
        } catch (error) {
    ...
    async updateMember(code, name) {
        try {
            const updatedMember = await Member.findOneAndUpdate({ code }, { name }, { new: true });
    ...
    async deleteMember(code) {
        try {
            return await Member.findOneAndDelete({ code });
        } catch (error) {
    ...
    async addBookBorrowed(code, bookCode, copyCode) {
        try {
            ...
            if (duplicateCount >= 1) {
                throw new Error(`Member with code ${code} cannot borrow the same book`);
            }
